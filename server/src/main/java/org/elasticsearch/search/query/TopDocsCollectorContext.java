...
abstract class TopDocsCollectorContext extends QueryCollectorContext {
    ...
    abstract static class SimpleTopDocsCollectorContext extends TopDocsCollectorContext {
        ...
        private final Collector collector; // SimpleTopDocsCollector
        ...
        private SimpleTopDocsCollectorContext(IndexReader reader,
            ...
            if ((sortAndFormats == null || SortField.FIELD_SCORE.equals(sortAndFormats.sort.getSort()[0]))
                ...
                if (hitCount == -1) {
                    topDocsCollector = createCollector(sortAndFormats, numHits, searchAfter, trackTotalHitsUpTo); // SimpleTopDocsCollectorContext
                    topDocsSupplier = new CachedSupplier<>(topDocsCollector::topDocs);
                    totalHitsSupplier = () -> topDocsSupplier.get().totalHits;
                } else {
                    // don't compute hit counts via the collector
