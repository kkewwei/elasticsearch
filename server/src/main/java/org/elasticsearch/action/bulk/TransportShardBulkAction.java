...
public class TransportShardBulkAction extends TransportWriteAction<BulkShardRequest, BulkShardRequest, BulkShardResponse> {
    ...
    protected void shardOperationOnPrimary(BulkShardRequest request, IndexShard primary,
        ...
        performOnPrimary(request, primary, updateHelper, threadPool::absoluteTimeInMillis, // 进来
    ...
    public static void performOnPrimary(
        ...
        MappingUpdatePerformer mappingUpdater, // 更新mapping
        Consumer<ActionListener<Void>> waitForMappingUpdate,// 等待完成
        ActionListener<PrimaryResult<BulkShardRequest, BulkShardResponse>> listener,
        ...
        new ActionRunnable<PrimaryResult<BulkShardRequest, BulkShardResponse>>(listener) {
            ...
            protected void doRun() throws Exception {
                while (context.hasMoreOperationsToExecute()) { // 会循环多次执行
            ...
            private void finishRequest() {
                ActionListener.completeWith(listener, // lister定义详见ReplicationOperation.execute,主要定义了写完主本后的操作（写副本）
                    () -> new WritePrimaryResult<>( //这个函数比较特殊，当WritePrimaryResult对象构建完成后，会去调用
    ...
    static boolean executeBulkItemRequest(BulkPrimaryExecutionContext context, UpdateHelper updateHelper, LongSupplier nowInMillisSupplier,
        ...
        if (isDelete) {
            ...
            result = primary.applyIndexOperationOnPrimary(version, request.versionType(), new SourceToParse( // 开始向lucene写入索引结构
        ...
        if (result.getResultType() == Engine.Result.Type.MAPPING_UPDATE_REQUIRED) { // mapping更新的话，在前一步就没有真正调用lucene写入
            ...
            mappingUpdater.updateMappings(result.getRequiredMappingUpdate(), primary.shardId(),
                ...
                new ActionListener<Void>() {
                    ...
                    public void onResponse(Void v) {
                        ...
                        waitForMappingUpdate.accept( // 更新完mapping后会再调用TransportShardBulkAction.performOnPrimary()里面定义的匿名类，进而执行本函数（写入）
