...
public class TransportShardBulkAction extends TransportWriteAction<BulkShardRequest, BulkShardRequest, BulkShardResponse> {
    ...
    public static void performOnPrimary(
        ...
        new ActionRunnable<PrimaryResult<BulkShardRequest, BulkShardResponse>>(listener) {
            ...
            private final Executor executor = threadPool.executor(ThreadPool.Names.WRITE); //write线程
    ...
    static boolean executeBulkItemRequest(BulkPrimaryExecutionContext context, UpdateHelper updateHelper, LongSupplier nowInMillisSupplier,
        ...
        final DocWriteRequest.OpType opType = context.getCurrent().opType(); // Index
        ...
        // 只要把request放入Context的requestToExecute才是开始写入的地方
        assert context.getRequestToExecute() != null; // also checks that we're in TRANSLATED state

        final IndexShard primary = context.getPrimary();
        final long version = context.getRequestToExecute().version(); // 参考Version.java里面的介绍，-3表示不用考虑version问题
        ...
        if (result.getResultType() == Engine.Result.Type.MAPPING_UPDATE_REQUIRED) { // 会跑到else里面
    ...
    private static void onComplete(Engine.Result r, BulkPrimaryExecutionContext context, UpdateHelper.Result updateResult) {
        ...
        final DocWriteRequest.OpType opType = docWriteRequest.opType(); //Index
